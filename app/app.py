...
# vLLM endpoint configuration from environment variables (set these if using vLLM server)
# For local run without vLLM, leave as None (default)
...
# Device detection: Supports NVIDIA (CUDA), MacBook (MPS), or CPU
device = "cuda" if torch.cuda.is_available() else "mps" if torch.backends.mps.is_available() else "cpu"

# Choose best attention implementation and dtype based on device
if device == "cuda":
    attn_implementation = "sdpa"
    dtype = torch.bfloat16
    print("Using sdpa attention and bfloat16 for CUDA (NVIDIA GPU)")
elif device == "mps":
    attn_implementation = "eager"
    dtype = torch.float32  # bfloat16 is supported but float32 is more stable on MPS
    print("Using eager attention and float32 for MPS (Apple Silicon MacBook)")
else:
    attn_implementation = "eager"
    dtype = torch.float32
    print("Using eager attention and float32 for CPU")
...
def extract_text_via_vllm(image, model_name, temperature=0.2, stream=False, max_tokens=2048):
    """Extract text from image using vLLM endpoint (if configured)."""
    config = MODEL_REGISTRY.get(model_name)
    if config is None:
        raise ValueError(f"Unknown model: {model_name}")
    ...
    if endpoint is None:
        raise ValueError(f"Model {model_name} does not have a vLLM endpoint configured. Set the environment variable.")
...
def extract_text_from_image(image, model_name, temperature=0.2, stream=False, max_tokens=2048):
    """Extract text from image using LightOnOCR model (local or vLLM)."""
    # Check if model has a vLLM endpoint configured
    config = MODEL_REGISTRY.get(model_name, {})
    if config.get("vllm_endpoint"):
        # Use vLLM endpoint if available
...
# Create Gradio interface with soft theme for polished aesthetic
